...
function displayCartelaRanges() {
    ...
    cartelaRanges.forEach((faixa, index) => {
        ...
        div.className = 'flex items-center justify-between w-full h-8 bg-gray-800 rounded-lg text-xl text-gray-400 font-bold';
        div.innerHTML = `
            <span>P${index + 1}: ${faixa.inicial} - ${faixa.final}</span>
            <button class="remover-faixa px-2 h-8 bg-red-800 rounded-md text-sm text-gray-300 font-normal " data-index="${index}">Remover</button>
        `;
