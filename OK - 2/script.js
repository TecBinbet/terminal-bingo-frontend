...
function setupCartelasEmJogo(maxCardNumber) {
    ...
    adicionarBtn.addEventListener('click', () => {
        ...
        if (novaSoma > maxCartelas) {
            ...
            validationMessageCurrent.textContent = `Erro: O total de cartelas (${novaSoma}) excede o máximo permitido (${maxCartelas}).`;
    ...
    loadedCards.forEach(card => {
        ...
        if (isMultiLinePrize) {
            ...
            if (normalizedPremioBuscado.includes('QUADRA') && count === 4) {
                ...
                displayPrizeAlert('LINHA' );
                console.error("Linha: ", card.cartao);
            }
        } else {
            let count = 0;
...
function displayLoadedCards(bolasCantadas) {
    ...
    if (cardsToDisplay.length === 0) {
        ...
        cardsToDisplay.forEach(card => {
            ...
            cardDiv.className = 'flex w-full p-0 bg-gray-800 rounded-lg text-white font-medium mb-0';
            ...
            numbersContainer.className = 'flex-1 ml-2 p-2 bg-gray-900 rounded-lg flex flex-wrap gap-1 justify-start';
            ...
            if (card.premioEncontrado) {
                ...
                premioSpan.className = 'text-3xl bg-red-500 text-white font-bold w-full text-center p-2 rounded-lg animate-blink-red-white';
                ...
                missingNumbers.forEach((num, index) => {
                    ...
                    const numberClass = `p-2 rounded-lg text-white font-bold ${bgColorClass} text-xl w-10 h-8 flex items-center justify-center flex-shrink-0`;
...
if (toggleCartelasButton && mobileCartelasContent) {
    toggleCartelasButton.addEventListener('click', () => {
        mobileCartelasContent.classList.toggle('hidden');
        if (mobileCartelasContent.classList.contains('hidden')) {
...
function displayPrizeAlert(prizeName) {
    ...
    // Se o prêmio já foi alertado, não faz nada
    if (alertedPrizes.includes(prizeName)) {
        return;
    }

    // Limpa qualquer timeout anterior para evitar múltiplos alertas
    if (alertTimeout) {
        clearTimeout(alertTimeout);
    }

    // Preenche o conteúdo do alerta apenas com o nome do prêmio
    alertPrizeName.textContent = prizeName;

    // Se você não precisa exibir as cartelas vencedoras, pode limpar o conteúdo
    ...
    // Adiciona o prêmio à lista de prêmios já alertados
    ...
    alertTimeout = setTimeout(() => {
        ...
        setTimeout(() => {
            prizeAlert.classList.add('hidden');
        }, 500);
    }, 5000);
}
